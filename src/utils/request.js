...
service.interceptors.request.use(
  (config) => {
    ...
    if (getToken() && !isToken) {
      ...
      config.headers["sysId"] = localStorage.getItem("sysId");
    }
    // get请求映射params参数
...
// 通用下载方法
export function download_oss(url, filename, config) {
  downloadLoadingInstance = Loading.service({
    text: "正在下载数据，请稍候",
    spinner: "el-icon-loading",
    background: "rgba(0, 0, 0, 0.7)",
  ...
  return new Promise((resolve, reject) => {
    service
      ...
      .catch((r) => {
        console.error(r);
        reject(r);
        Message.error("文件上传出现错误，请联系管理员！");
        downloadLoadingInstance.close();
      });
  });
}

export function request(uri, datas, method = "post") {
  let data, params;
